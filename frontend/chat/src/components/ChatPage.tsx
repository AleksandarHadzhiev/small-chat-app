...
export default function Chat({ groupName }) {
    ...
    function showMessage(message: string) {
        ...
        if (data.group === groupName && data.tag === "send message") {
            const messageContainer = document.getElementById("messages")?.lastElementChild;
            const messageElement = document.createElement('p');
            messageElement.textContent = data.content
            messageContainer?.append(messageElement)
        }
        else if (data.tag == "typing") {
            const messageContainer = document.getElementById("send-message")
            if (messageContainer) {
                const typingElement = document.createElement('div')
                typingElement.className = "w-full bg-gray-400 text-white"
                typingElement.textContent = data.email + ' is typing ...'
                typingElement.setAttribute("id", "typing")
                const last = messageContainer.lastChild
                if (last)
                    messageContainer.insertBefore(typingElement, last)
                else
                    messageContainer.appendChild(typingElement)
            }
        }
        else if (data.tag == "stopped typing") {
            const messageContainer = document.getElementById("typing")
            if (messageContainer)
    ...
    function sendMessage(e: FormEvent<HTMLFormElement>) {
        ...
        setMessage("")
        socket.send(JSON.stringify({ content: content, group: groupName }))
    }
    ...
    function displayTyping(e: FocusEvent<HTMLInputElement, Element>) {
        e.preventDefault()
        const notification = {
            type: {
                title: 'notification',
        ...
        socket.send(JSON.stringify({ content: notification, group: groupName }))
    }

    function removeTyping(e: FocusEvent<HTMLInputElement, Element>) {
        e.preventDefault()
        const notification = {
            type: {
                title: 'notification',
        ...
        <div className="flex flex-col w-3/4 h-full">
            {groupName !== "" ? (
                <div className="h-full w-full">
                    ...
                    <form id="send-message" onSubmit={(e) => { sendMessage(e) }} className="flex flex-col w-full h-1/10 bg-gray-900 text-white">
                        <div className="flex w-full h-full">
                            <input
                                onFocus={(e) => { displayTyping(e) }}
                                onBlur={(e) => { removeTyping(e) }}
